...
## 프로젝트 정보 (핏기어 / dietexercise7)

- **무엇**: 한국어 피트니스 **장비·앱 리뷰** 블로그(`dietexercise7.com`, 브랜드명 "핏기어"). gameworld7 틀을 복제해 만든 수익형(쿠팡파트너스 제휴) 사이트.
- **니치 근거**: 다이어트/운동이라 YMYL처럼 보이지만 **제품·앱 리뷰**로 각도를 틀면 비YMYL이라 안전 + 쿠팡 제휴 수익. ⚠️ 의학·건강 조언(식단·보충제 효능·치료) 금지, "사는 물건/앱"만 리뷰.
- **디자인**: gameworld7·spokdom의 다크 네온과 다르게 **라이트 테마 + 코랄/오렌지 포인트**(global.css). 둥근 카드 + 소프트 섀도우. accent `#ff5a3c`.
- **스택**: Astro + Markdown + Cloudflare Pages. 본문 **한국어**.
- **배포**: Git 연동이 아니라 **wrangler 직접 업로드**(finalize.mjs가 빌드·커밋·푸시 후 `wrangler pages deploy dist --project-name=dietexercise7`까지 실행). wrangler는 계정에 이미 로그인됨.
- **자매 사이트**: gameworld7(게임)·sportskingdom24(스포츠)·click-gotraveling(여행)·spokdom(영어 SaaS 리뷰). 같은 자동발행 틀 공유.
